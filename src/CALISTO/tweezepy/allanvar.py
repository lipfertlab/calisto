...
def calc_avar(phase, rate, mj, step):
    ...
    n0 = (phase.shape[0] - 0 * mj_i + step_i - 1) // step_i
    n1 = (phase.shape[0] - 1 * mj_i + step_i - 1) // step_i
    n2 = (phase.shape[0] - 2 * mj_i + step_i - 1) // step_i
    ...
    for idx in range(n):
        ...
        i1 = base1 + idx * step_i
        i2 = base2 + idx * step_i
        diff = phase[i2] - 2.0 * phase[i1] + phase[i0]
        s += diff * diff
...
def avar(data, rate=1.0, taus="octave", overlapping=True, edf="approx"):
    ...
    if overlapping:
        steps = np.ones_like(m_int)
        oavs = calc_avar_batch(phase, rate, m_int, steps)
...
def noise_id(x, af, dmin=0, dmax=2):
    ...
    if N < 32:
        warn(
            ("noise_id() Can't determine noise-ID for time-series of length= %d")
            % len(x)
        )
...
def edf_greenhall(alpha, d, m, N, overlapping=False, modified=False, verbose=False):
    ...
    if int(F) == 1 and modified:  # case 1, modified variances, all alpha
        if J <= J_max:
            inv_edf = (
                1.0 / (pow(greenhall_sz(0, 1, alpha, d), 2) * M)
            ...
            m_prime = J_max / r
            inv_edf = (
                1.0 / (pow(greenhall_sz(0, F, alpha, d), 2) * J_max)
        ...
        if J <= J_max:
            ...
            inv_edf = (
                1.0 / (pow(greenhall_sz(0, m_prime, alpha, d), 2) * M)
        ...
        if J <= J_max:
            ...
            inv_edf = (
                1.0 / (pow(greenhall_sz(0, m, 1, d), 2) * M)
        ...
        if K <= d:
            inv_edf = 1.0 + (2.0 / scipy.special.binom(2 * d, d)) * np.sum(
                [
                    (1.0 - k / r) * pow(scipy.special.binom(2 * d, d - k), 2)
                    for k in range(1, K)
                ]
            )
            if verbose:
            ...
            a0 = scipy.special.binom(4 * d, 2 * d) / pow(
                scipy.special.binom(2 * d, d), 2
            )
            a1 = d / 2.0
...
def greenhall_BasicSum(J, M, S, F, alpha, d):
    ...
    second = (1 - float(J) / float(M)) * pow(
        greenhall_sz(float(J) / float(S), F, alpha, d), 2
    )
    third = 0
