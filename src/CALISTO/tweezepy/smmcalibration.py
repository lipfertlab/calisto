...
from tweezepy.hadamardvar import hvar
from tweezepy.MLE import MLEfit, Gamma_Distribution
from tweezepy.expressions import aliasPSD, lansdorpPSD, SMMAV, SMMHV, HV_noise
...
class calibration(MLEfit):
    ...
    def plot(
        ...
        if "yfit" in data.keys():
            ...
            ax[0].plot(
                data["x"], data["yfit"], lw=2, label=fit_label, c=fit_color, zorder=1
            )
            if len(ax) == 2:
                ax[1].plot(data["x"], data["residuals"], c=data_color)
                ax[1].set_xscale("log")
                ax[0].get_xaxis().set_ticklabels([])
        if isinstance(self, AV):
            ax[-1].set_xlabel(r"$\tau$ (s)")
            ax[0].set_ylabel(r"$\sigma_{AV}^2$ (nm$^2$)")
        if isinstance(self, HV):
            ax[-1].set_xlabel(r"$\tau$ (s)")
            ax[0].set_ylabel(r"$\sigma_{HV}^2$ (nm$^2$)")
        if isinstance(self, PSD):
            ax[-1].set_xlabel(r"$f$ (Hz)")
            ax[0].set_ylabel(r"PSD (nm$^2$/Hz)")
        return fig, ax

    def _make_guess(self, kT=4.1, viscosity=8.94e-10, radius=530):
        """
        ...
        kappa = kT / np.var(detrend(self.trace))
        # Make initial guess for alpha based on myone bead in water
        gamma = 6 * np.pi * viscosity * radius
        guess = [gamma, kappa]
        return guess

    def _make_noise_guess(self, noise, fsample):
        """
        Estimate noise parameters based on the noise power spectral density.
        """
        psd = PSD(noise, fsample)
        A = np.vstack(
            [np.ones_like(psd.data["x"]), 1 / psd.data["x"], 1 / psd.data["x"] ** 2]
        ).T
        w = 1 / np.sqrt(psd.data["yerr"] + 1e-8)
        A = np.einsum("i,ij->ij", w, A)
        y = psd.data["y"] * w

        guess, _, _, _ = np.linalg.lstsq(A, y, rcond=None)
        guess[guess < 0] = 0
    ...
    def _predefined(
        ...
        elif epsilon is not None and tracking_error:
    ...
    def mlefit(
        ...
        if cutoffs:
            assert (
                len(cutoffs) == 2
            ), "cutoffs should have a lower an upper cutoff in the form of [lower,upper]."
        ...
        if isinstance(self, HV):
            ...
            if fitfunc == "SMMHV":
                ts = 1 / self.fsample
                func = lambda g, k, e: SMMHV(x, ts, g, k, e, kT=kT)
                func = self._predefined(
                    func,
                    gamma=gamma,
                    kappa=kappa,
            ...
            elif fitfunc == "noiseHV":
                ts = 1 / self.fsample
                func = lambda h_m2, h_m1, h0: HV_noise(x, ts, h_m2, h_m1, h0)
                minimizer_kwargs.update(
                    {"bounds": [(0, np.inf), (0, np.inf), (0, np.inf)]}
                )
                if not guess:
                    guess = self._make_noise_guess(self.trace, self.fsample)
            else:
                func = fitfunc
                if not guess:
                    raise RuntimeError(
        ...
        if not guess:
            ...
            guess = self._make_guess(kT=kT, viscosity=viscosity, radius=radius)
            if gamma:
                guess.pop(0)
            if kappa:
                guess.pop(1)
            if tracking_error and epsilon is None:
                guess.append(0.0)
        self.guess = guess
        MLEfit.__init__(
            self,
...
class AV(calibration):
    ...
    def __init__(self, trace, fsample, taus="octave", mode="oavar", edf="real"):
        ...
        assert taus in [
            ...
            "octave",
            "decade",
        ], "taus must be either all, octave, or decade."
        assert mode in [
            "avar",
            "oavar",
            "totvar",
        ], "mode must be either avar, oavar, or totvar."
        ...
        self._data = {"x": self.x, "shape": self.shape, "y": self.y, "yerr": self.yerr}
...
################################################################
# Hadamard modifications start here
################################################################


class HV(calibration):
    """
    A class for computing and fitting the Hadamard variance using maximum likelihood estimation.
    ...
    taus : str, optional
        Tau interval sampling. Options are 'octave', 'decade', and 'all', by default 'octave'
    mode : str, optional
        Hadamard variance type, either 'hvar', 'ohvar', and 'totvar', by default 'ohvar'.
        See hadamardvar module for more information.
    edf : str, optional
        Equivalent degrees of freedom for HV. Options are 'real' and 'approx', by default 'real'
    ...
    def __init__(self, trace, fsample, taus="octave", mode="ohvar", edf="real"):
        calibration.__init__(self, trace, fsample)
        assert taus in [
            "all",
            "octave",
        ...
        assert mode in [
            ...
            "ohvar",
            "totvar",
        ], "mode must be either hvar, ohvar, or totvar."
        assert edf in ["real", "approx"], "edf must be either real or approx."
        if mode == "hvar":
            taus, edfs, ohvs = hvar(
                trace, rate=fsample, taus=taus, edf=edf, overlapping=False
